...
def install_qos_flow(switch_id, flow_id, priority, src_ip, dst_ip, queue_id):
    """Instala flow con QoS - marca trafico hacia una cola especifica"""
...
def install_priority_flow(switch_id, flow_id, priority, src_ip, dst_ip):
    """Flow de alta prioridad - reenvio directo sin cola (para VOIP/critico)"""
    url = f"{BASE_URL}/opendaylight-inventory:nodes/node/{switch_id}/table/0/flow/{flow_id}"
...
def apply_isp_qos_policies():
    """
    Politicas QoS para topologia ISP:
    - Zona 1 (h1-h5)   clientes residenciales - prioridad normal
    - Zona 2 (h6-h10)  clientes empresariales - prioridad alta
    - Zona 3 (h11-h15) servidores             - prioridad critica
    - Zona 4 (h16-h20) gestion                - prioridad maxima
    """
    print("\n=== Aplicando politicas QoS ISP ===\n")

    # Trafico desde servidores (zona 3) hacia clientes - prioridad critica
    ...
    # Trafico desde zona empresarial (h6-h10) - prioridad alta
    for switch_id in ["openflow:1", "openflow:2", "openflow:3", "openflow:4"]:
        for i in range(6, 11):
    ...
    print("\n=== Politicas QoS aplicadas ===")
    print("  Servidores  (h11-h15) -> prioridad 200 (critica)")
    print("  Empresarial (h6-h10)  -> prioridad 150 (alta)")
    print("  Residencial (h1-h5)   -> prioridad default")
    print("  Gestion     (h16-h20) -> prioridad default\n")
